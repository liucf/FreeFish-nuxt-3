                ...
                <div class="mt-6 space-y-12 lg:grid lg:grid-cols-3 lg:gap-x-6 lg:space-y-0 lg:gap-y-6">
                    <div v-for="collection in collections" :key="collection.name" class="group relative">
                        <div
                            ...
                            <img :src="collection.thumb" :alt="collection.name"
                                class="h-full w-full object-cover object-center" />
                        </div>
                        <h3 class="mt-6 text-sm text-gray-500">
                            <a :href="collection.href">
                                <span class="absolute inset-0" />
                                {{ collection.parent_id.name }}
                            </a>
                        </h3>
                        <p class="text-base font-semibold text-gray-900">{{ collection.name }}</p>
                    </div>
                </div>
            </div>
        </div>
...
//     },
// ]
const { data: collections } = await useFetch('category_2', { baseURL: useRuntimeConfig().public.BASE_URL })
